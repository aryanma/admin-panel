...
export default async function LlmPromptChainsPage({
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-zinc-400">All prompt chains ({count ?? 0})</p>
      <div className="mt-6 overflow-x-auto rounded-lg border border-zinc-800">
        <table className="w-full text-left text-sm">
          <thead className="border-b border-zinc-800 bg-zinc-900 text-zinc-400">
            <tr>
              <th className="px-4 py-3 font-medium">ID</th>
              <th className="px-4 py-3 font-medium">Caption Request ID</th>
      ...
      <Pagination page={page} totalPages={Math.ceil((count ?? 0) / PAGE_SIZE)} basePath="/llm-prompt-chains" />
    </div>
  );
}
