...
export default async function CaptionRequestsPage({
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-zinc-400">All caption requests ({count ?? 0})</p>
          ...
          <tbody className="divide-y divide-zinc-800">
            {rows?.map((r) => (
              <tr key={r.id} className="hover:bg-zinc-900/50">
                ...
                <td className="max-w-[200px] truncate px-4 py-3 text-zinc-300">{r.profile_id}</td>
                <td className="max-w-[200px] truncate px-4 py-3 text-zinc-300">{r.image_id}</td>
      ...
      <Pagination page={page} totalPages={Math.ceil((count ?? 0) / PAGE_SIZE)} basePath="/caption-requests" />
    </div>
  );
}
