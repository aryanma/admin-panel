...
export default async function LlmResponsesPage({
  ...
  const [{ data: rows }, { count }] = await Promise.all([
    supabase
      ...
      .select("id, llm_model_id, humor_flavor_id, processing_time_seconds, caption_request_id, created_datetime_utc")
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-zinc-400">All LLM model responses ({count ?? 0})</p>
          ...
          <tbody className="divide-y divide-zinc-800">
            {rows?.map((r) => (
              <tr key={r.id} className="hover:bg-zinc-900/50">
                ...
                <td className="px-4 py-3 text-zinc-300">{r.processing_time_seconds?.toFixed(2) ?? "—"}</td>
                <td className="px-4 py-3 text-zinc-300">{r.caption_request_id}</td>
      ...
      <Pagination page={page} totalPages={Math.ceil((count ?? 0) / PAGE_SIZE)} basePath="/llm-responses" />
    </div>
  );
}
